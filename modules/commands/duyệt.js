...
module.exports.handleReply = async function({ api, event, handleReply, getText }) {
    ...
    if (isNaN(body) && body.indexOf("c") == 0 || body.indexOf("cancel") == 0) {
        ...
        const fs = require("fs");       
        for (const singleIndex of index) {
            if (isNaN(singleIndex) || singleIndex <= 0 || singleIndex > handleReply.pending.length) return api.sendMessage(`❯ ${singleIndex} Không PhảiLlà Một Con Số Hợp Lệ`, threadID, messageID);
