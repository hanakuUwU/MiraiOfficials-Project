	...
	if (urlValid) {
		try { 
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.m4a`).size > 26214400) return api.sendMessage('⚡𝐁𝐚̀𝐢 𝐍𝐚̀𝐲 𝐃𝐚̀𝐢 𝐐𝐮𝐚́, 𝐂𝐡𝐮𝐲𝐞̂̉𝐧 𝐁𝐚̀𝐢 𝐊𝐡𝐚́𝐜 𝐃𝐢 🥺', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.m4a`), event.messageID);
					else{
			...
			console.log(e);
			api.sendMessage("⚡𝐊𝐡𝐨̂𝐧𝐠 𝐓𝐡𝐞̂̉ 𝐗𝐮̛̉ 𝐋𝐢́ 𝐘𝐞̂𝐮 𝐂𝐚̂̀𝐮 𝐁𝐚̀𝐢 𝐍𝐚̀𝐲 🥺", event.threadID, event.messageID);
		}
	}
	else {
		try {
